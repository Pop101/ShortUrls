...
// Get basic fingerprint
fp.screen = {width: screen.width, height: screen.height}
fp.screen_avail = {width: screen.availWidth, height: screen.availHeight};
fp.time_zone = new Date().getTimezoneOffset();
fp.build_id = navigator.buildID ? navigator.buildID : null;
fp.dnt = navigator.doNotTrack ? navigator.doNotTrack : null;
fp.locality = window.navigator.userLanguage || window.navigator.language;
fp.fire_gloves = ((typeof (ntrn) == "function" && typeof (ntra) == "function") || (typeof (FireGlovesStats) == "object")) ? true : false

// Get Fonts
let det = new Detector();
for (i = 0; i < fonts.length; i++)
    if(det.detailedTest(fonts[i])[3]) fp.fonts.push(fonts[i])

// Get plugins (doesn't really work...)
try {
    var pNames = ['Silverlight', 'VLC', 'Java', 'Shockwave', 'QuickTime', 'Flash', 'AdobeReader', 'WindowsMediaPlayer'];
    PluginDetect.getVersion('.');
    for (var i in pNames)
        if(PluginDetect.isMinVersion(pNames[i], '0') >= 0)
            fp.plugins[pNames[i]] = PluginDetect.getVersion(pNames[i])
}
catch (e) {}
...
if(navigator.mimeTypes)
    for (var i in navigator.mimeTypes) {
        let plugin = navigator.mimeTypes[i].enabledPlugin == null ? null : {
            name: navigator.mimeTypes[i].enabledPlugin.name,
            version: navigator.mimeTypes[i].version,
            filename: navigator.mimeTypes[i].enabledPlugin.filename
        }
        fp.mimetypes.push({
...
// Attempt to get entire window
// Thank you to https://stackoverflow.com/questions/37697172/converting-entire-window-object-to-string
...
let w_str = JSON.stringify(window, getCircularReplacer());
fp.window_hash = SHA256(w_str)*/
try {
    // Grab Canvas
    ...
    ctx.fillStyle = "rgba(156, 48, 61, 0.456)";
    ctx.fillText(txt, 4, 17);
    ctx.shadowBlur=9;
    ctx.shadowColor="green";
    ctx.fillRect(-20,10,234,5);
    fp.canvas_hash = SHA256(canvas.toDataURL().replace("data:image/png;base64,",""));
}
catch (e) {}
try{
    // Grab Navigator
    ...
    fp.navigator_hash = SHA256(JSON.stringify(_navigator))
}
catch (e) {}
...
// POST fingerprint to current URL
